...
<details>
  ...
  <p>Protobuf 사용 시 파싱이 필요없는 것은 아니고, JSON 사용 시 문자열을 파싱하지만 Protobuf는 binary data를 파싱해야합니다.</p>
</details>
...
<details>
  ...
  <p>Protobuf 사용 시 파싱이 필요없는 것은 아니고, JSON 사용 시 문자열을 파싱하지만 Protobuf는 binary data를 파싱해야합니다.
gRPC는 가장 널리 사용되는 HTTP/JSON과 자주 비교됩니다 (HTML/JSON이라고 오타가 난 것 같네요)
gRPC는 HTTP/2 기반으로 동작하고, Bi-directional streaming을 지원한다는 점에서 (일반적으로 사용되는) HTTP/1.1보다 우수한 성능을 보입니다.
</p>
</details>
